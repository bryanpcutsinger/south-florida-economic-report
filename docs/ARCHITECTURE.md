...
**2026-06-23** — Trend-chart hover cleanup + quarter-labeled x-axis.
- `components/employment_trends.py` (`_build_chart`) and `build.py` (`_trends_chart`): switched `hovermode` from `"x unified"` to `"closest"` so a hover shows only the single nearest point (actual quarters show the trend value; projected points show the projection) instead of a stacked box; removed the `%{fullData.name}` ("Trend:"/"Projected:") prefix from the shared `hovertemplate`; relabeled the projection connector point from "Latest trend" to the actual last quarter so a boundary hover reads consistently; added `tickformat="%Y Q%q"` to the x-axis so ticks read as quarters ("2025 Q3") rather than auto month labels (`%q` is a Plotly-added directive, supported in both the local 6.7.0 app and the CDN-pinned `plotly-2.35.2.min.js` static build).
- `utils/formatting.py`: added `fmt_quarter_label(ts, projected=False)` and used it in both chart builders, replacing the duplicated `{2:1,5:2,8:3,11:4}[d.month]` month→quarter mapping for projected-point labels.
